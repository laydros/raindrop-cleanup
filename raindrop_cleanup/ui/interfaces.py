...
class UserInterface:
        ...
        if can_use_curses:
            try:
                ...
                if hasattr(curses, 'wrapper') and os.isatty(0):  # Check if stdin is a terminal
                    return self._display_keyboard_interface(bookmarks, decisions)
                else:
                    print("⚠️  Terminal doesn't support interactive mode, falling back to text interface")
                    return self._display_text_interface(bookmarks, decisions)
    ...
    def _display_keyboard_interface(self, bookmarks: List[Dict], decisions: List[Dict]) -> List[int]:
        ...
        def draw_interface(stdscr):
            ...
            while True:
                ...
                if key in [ord('j'), curses.KEY_DOWN]:
                    ...
                    available_actions = self._get_available_actions(decisions[current_bookmark])
    ...
    def _display_text_interface(self, bookmarks: List[Dict], decisions: List[Dict]) -> List[int]:
