...
class TestCLIMain:
    ...
    def test_resume_option_new_session(
        self, mock_input, mock_cleaner, mock_collections
    ):
    ...
    )  # Select collection 1, then press Enter
    def test_normal_processing_flow(self, mock_input, mock_cleaner, mock_collections):
        ...
        mock_cleaner_instance.raindrop_client.find_collection_by_name.return_value = (
            4  # Archive ID
        )
        mock_cleaner.return_value = mock_cleaner_instance
...
class TestCLIHelpers:
    ...
    def test_handle_resume_selection_valid_number(self):
        ...
        sessions = [{"collection_name": "Test 1"}, {"collection_name": "Test 2"}]
    ...
    def test_resume_session_integration(
        self, mock_input, mock_resume_session, mock_collections
    ):
        """Test resuming a session integration."""
        sessions = [{"collection_id": 1, "collection_name": "Test Collection"}]
...
class TestArgumentParsing:
    ...
    def test_batch_size_argument(self, mock_input, mock_cleaner):
        ...
        with patch(
            "raindrop_cleanup.cli.main._select_collection",
            return_value={"_id": 1, "title": "Test", "count": 10},
        ):
            with patch(
                "builtins.input", side_effect=["", ""]
            ):  # Skip dry-run warning, start processing
                main()
