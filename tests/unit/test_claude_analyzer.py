...
class TestClaudeAnalyzer:
    ...
    def test_rate_limit_applies_delay(self, mock_sleep, mock_anthropic_client):
        ...
        with patch('raindrop_cleanup.ai.claude_analyzer.time.time', side_effect=[0.5, 0.5]):
            analyzer._rate_limit()
        
        mock_sleep.assert_called_once_with(0.5)  # Should sleep for remaining 0.5 seconds
    ...
    def test_rate_limit_no_delay_needed(self, mock_sleep, mock_anthropic_client):
        ...
        with patch('raindrop_cleanup.ai.claude_analyzer.time.time', side_effect=[2.0, 2.0]):
            analyzer._rate_limit()
    ...
    def test_build_collection_info(self, mock_anthropic_client, mock_collections):
        ...
        analyzer = ClaudeAnalyzer(client=mock_anthropic_client)
        collection_info = analyzer._build_collection_info(mock_collections, "Development")
...
4. ARCHIVE - historical document"""
        ...
        assert decisions[1] == {'action': 'MOVE', 'target': 'Gaming', 'reasoning': 'game guide'}
    ...
    def test_parse_batch_response_insufficient_responses(self, mock_anthropic_client):
        ...
        assert decisions[1] == {'action': 'KEEP', 'reasoning': 'no recommendation received'}
        assert decisions[2] == {'action': 'KEEP', 'reasoning': 'no recommendation received'}
    
    @patch('raindrop_cleanup.ai.claude_analyzer.time.time')
    def test_analyze_batch_success(self, mock_time, mock_anthropic_client, mock_bookmarks, mock_collections):
        """Test successful batch analysis."""
        mock_time.return_value = 100.0
        
        mock_message = Mock()
        mock_message.content = [Mock()]
        mock_message.content[0].text = """1. MOVE:Development - programming tutorial
2. MOVE:Gaming - game guide  
3. DELETE - outdated content"""
    ...
    @patch('raindrop_cleanup.ai.claude_analyzer.time.time')
    def test_analyze_batch_api_error(self, mock_time, mock_anthropic_client, mock_bookmarks):
